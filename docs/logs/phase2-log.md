...
5. **Phân quyền Bảo mật (Authorization)**:
   ...
   - Tiến hành check Session: Nếu chưa đăng nhập hoặc Role không hợp lệ -> Redicrect về trang Login kèm Error Message từ chối quyền truy cập. 
...
- **Views**: 
  ...
  - Thêm `web/admin/dashboard.jsp`, `web/admin/manage-categories.jsp`, `web/admin/manage-flowers.jsp`, `web/admin/manage-accounts.jsp`.

## Kinh nghiệm rút ra & Ghi chú
...
Chuyển hướng sang xây dựng luồng mua hàng (Nghiệp vụ cốt lõi) bao gồm Giỏ hàng (Cart) và Thanh toán (Checkout) dành cho Customer.
